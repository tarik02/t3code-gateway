...
class T3CodeWebIndexError extends Schema.TaggedErrorClass<T3CodeWebIndexError>()(
  "T3CodeWebIndexError",
  {
    cause: Schema.Defect(),
  },
...
const gatewayCatalogBootstrap = String.raw`
await (async () => {
...
export const layer = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    yield* router.add("GET", "/", () =>
      Effect.try({
        try: () => {
          const html = fs.readFileSync(path.join(t3codeWebStaticRoot, "index.html"), "utf8");
          return HttpServerResponse.text(patchIndexHtml(html), {
            contentType: "text/html; charset=utf-8",
          });
        },
        catch: (cause) => new T3CodeWebIndexError({ cause }),
